...
fn decodeAll(allocator: std.mem.Allocator, data: []const u8) ![]f32 {
    var audio = try api.decodeMemory(allocator, data);
    defer audio.deinit(testing.allocator);

    const samples = std.mem.bytesAsSlice(f32, audio.data);
    const copy = try allocator.alloc(f32, samples.len);
    @memcpy(copy, samples);
    return copy;
}
...
test "QOA encode from WAV decodes equal to golden" {
    ...
    while (i < dec_golden.len) : (i += 1) {
        ...
        // Allow small floating point differences (about 1024/32768 = 0.031)
        try testing.expect(@abs(da - db) <= 0.032);
    }
}

// Note: Exact byte matching test removed after f32 refactor.
// The f32→i16 conversion introduces slight rounding differences in encoded output.
// The "QOA encode from WAV decodes equal to golden" test above verifies perceptual equivalence.
